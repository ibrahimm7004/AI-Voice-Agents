...
import { DietaryAccommodations } from "../../types/questions";
...
export const DietaryAccommodationsContent = ({
  question,
  callback,
}: {
  question: DietaryAccommodations;
  callback: () => void;
}) => {
  const [data, setData] = useState<DietaryAccommodations>({
    ...question,
  });
  ...
  return (
    <div className="flex gap-6 pb-6">
      <div className="flex flex-col gap-6 w-2/4">
        <RadioGroup
          label={"Do you offer dietary accommodations?"}
        ...
        <Input
          ...
          value={data.link}
          disabled
          onValueChange={(val) => setData({ ...data, link: val })}
        />
        <CheckboxGroup
      ...
      <Card className="w-2/4 p-3">
        ...
        <CardBody>
          <AudioPreview
            previewString={
              getPreviewText({
                type: question.prompt.toLowerCase() as "dietary_accommodations",
                ...data,
              }).string
            }
          />
